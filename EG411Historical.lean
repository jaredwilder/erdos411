...
/-!
# Historical EG#411 root

This is the pre-retraction import graph, preserved so old artifacts remain reproducible.
It includes the retracted Cambie branch and is not the live result target.

For the current mathematics, build `EG411Formal` (the default target), whose root now
imports only the live bridge, cascade, solution-structure, and omega-ladder modules.
-/
